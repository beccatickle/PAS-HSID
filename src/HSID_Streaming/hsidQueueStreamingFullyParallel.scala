...
object hsidQueueStreamingFullyParallel {

  val DELIMITER = "," // split data at this point
  val PERCENT_INIT = 10 // percentage of initial points in pre-stage1
  ...
  def main(arg: Array[String]) {
    ...
    val ssc = new StreamingContext(sc, Seconds(batchInterval))
     ...
     // input using filestream - Create a DStream by monitoring a given directory
  //  val rawIncidentStream: DStream[String] = ssc.textFileStream(inputDir)


    /***********************************************/
    // input using queueStream - load files into individual RDDs
...
//    val incidentSplitStream: DStream[Array[String]] = rawIncidentStream.map(line => line.split(DELIMITER))
    ...
    // this transform does stages 1-3
    val actualHotspots: DStream[((Double, Double), (Double, Array[String]))] = incidentLocationStream.transform(rdd => {
      ...
      /******************* If no hotspots exist yet, do pre-stage1 ***************/

      val currentHScount = hotspotCache.count()
      ...
      if(currentHScount == 0L) {
        ...
        // remove inital suppressor set from set of incidents
      ...
      val incidentsForNewHotspots: RDD[((Double, Double), (Int, Array[String], Boolean))] = countIncidentsRDD
        .filter(incident => incident._2._3).persist() // we keep the boolean tag for the FP version -- needed in parallel Stage 2
        //.map(incident => (incident._1, (incident._2._1, incident._2._2))).persist()
      ...
      /* ******************************************** STAGE 2 IN PARALLEL ******************************************** */
      /*  The parallel version of Stage 2 has two steps:
          2.A - clean each partition separately 
          2.B - compare partition 1 to 2..N, partition 2 to 3..N, etc. etc.
      * */

      /* **************** STEP 2.A **************** */
      println(">>>>>>>>>>>>>>>>>>>>> starting stage 2 parallel")

      var step2Result: RDD[(Int, Array[((Double, Double), (Int, Array[String], Boolean))])] = incidentsForNewHotspots.mapPartitionsWithIndex((index, incidents) =>
        removeRedundanciesInPartition(incidents, index, mileage)).persist()

      /* **************** STEP 2.B **************** */

      println("applying step 2B")

      for(i <- 0 until step2Result.getNumPartitions) {

        println("In iteration: "+i+ " | The number of partitions is: "+step2Result.getNumPartitions)

        val step2bCount = step2Result.count()

        var currentPartition: (Int, Array[((Double, Double), (Int, Array[String], Boolean))]) = step2Result.filterByRange(i,i).collect()(0)
        var subset: Array[((Double, Double), (Int, Array[String], Boolean))] = currentPartition._2 // get current suppressing subset
        println("Partition"+i+"has this many suppressors: "+subset.size)

        val subset_BC: Broadcast[Array[((Double, Double), (Int, Array[String], Boolean))]] = sc.broadcast(subset)
        println("Broadcast Var has this many suppressors: "+subset_BC.value.size)

        // clean all other partitions w.r.t current suppressing subset
        var newRDD: RDD[(Int, Array[((Double, Double), (Int, Array[String], Boolean))])] = step2Result.mapPartitionsWithIndex((index, info) => reduceStage2b(info, index, subset_BC, mileage, i))

        val newRDDcount = newRDD.count()
        println("At iter "+i+" newRDD.count is: "+newRDDcount)
      ...
      val parallelIncidentsNew: RDD[((Double, Double), (Int, Array[String]))] = step2Result.mapPartitions(dataset => unarrayPartition(dataset))

      val added = parallelIncidentsNew.count() // not really accurate anymore
      println("\n we are adding this many (candidate) hotspots to the state: "+added)

      /***************************************** end of stage 2 parallel ************************************/
      ...
      println("NO TIMEOUT")
        ...
        for(i <- 1 to numBatches) {
          rddQueue.synchronized{
            val inputI = inputFile + "_" + i + ".csv" //"e.g. HC_batch_"+i+".csv"
...
*/
  ...
  /** Used at the end of stage 2 to separate RDD(Array(k-v)) into RDD[k-v]
    *
    * @param iter iterator of partition contents
    */
  def unarrayPartition(iter: Iterator[(Int, Array[((Double, Double), (Int, Array[String], Boolean))])]): Iterator[((Double, Double), (Int, Array[String]))] = {

    println("IN UNARRAY PARTITION")

    var data: (Int, Array[((Double, Double), (Int, Array[String], Boolean))]) = iter.next() // iterator should only have one element
    if(iter.hasNext) {
      println("ERROR: Iterator has more than one value in unarrayPartition")
      System.exit(-1)
    }

    val arr: Array[((Double, Double), (Int, Array[String]))] = data._2.map(item => {
      val info = item._2._2
      info(INDEX_HS_AGE) = "0"
      (item._1, (item._2._1, info))
    })

    var outStr = "PART "+data._1+": "
    for(i <- 0 until arr.size) {
      outStr += "" + arr(i)._2._2(INDEX_HS_AGE)
    }
    println(outStr)

    val returnIter: Iterator[((Double, Double), (Int, Array[String]))] = arr.toIterator
  ...
  /** stage 2b
    * 
    * @param iter iterator of incidents/hotspots in this partition
    * @param index index of the current partition
    * @param currentHotspots_BC the current suppressing set of hotspots
    * @param mileage the hotspot radius
    * @param suppressingPartition Index of the suppressing partition
    */
  def reduceStage2b(iter: Iterator[(Int, Array[((Double, Double), (Int, Array[String], Boolean))])],
                    index: Int,
                    currentHotspots_BC: Broadcast[Array[((Double, Double), (Int, Array[String], Boolean))]],
                    mileage: Double,
                    suppressingPartition: Int): Iterator[(Int, Array[((Double, Double), (Int, Array[String], Boolean))])] = {

    if(!iter.hasNext) {
      println("ERROR: No incidents in iterator (partition "+index+")")
      System.exit(-1)
    }

    var data: (Int, Array[((Double, Double), (Int, Array[String], Boolean))]) = iter.next() // iterator should only have one element

    println("Suppressing: "+suppressingPartition+" | Suppressed: "+index+" | Partition incidents: "+data._2.size)

    if(iter.hasNext) {
      println("ERROR: Iterator has more than one value in reduceStage2b")
      System.exit(-1)
    }

    val thisPartitionIndex = data._1

    if(suppressingPartition < thisPartitionIndex) { // only suppress this partition if it is after the suppressing partition

      var currentHotspotsArr: Array[((Double, Double), (Int, Array[String], Boolean))] = currentHotspots_BC.value
      val incidentsIter: Iterator[((Double, Double), (Int, Array[String], Boolean))] = data._2.toIterator
      ...
      while(incidentsIter.hasNext) {
        val incident: ((Double, Double), (Int, Array[String], Boolean)) = incidentsIter.next()
        var closestHotspot: Option[((Double, Double), (Double, Array[String]))] = None // Option[((lat,lng),(miles, hotspot info))]


        breakable {

          if(incident._2._3) { // only compare incidents if they have a boolean flag TRUE (i.e. they are isolated incidents and don't represent a centre)
            for(i <- 0 until currentHotspotsArr.size) { // compare against all current hotspots

              // change structure of incident and hotspot so can use existing getMiles function
              val incidentGetMiles = (incident._1, incident._2._2)
              val hotspotGetMiles = (currentHotspotsArr(i)._1, currentHotspotsArr(i)._2._2)

              val miles = getMiles(incidentGetMiles, hotspotGetMiles) // calculate Haversine distance between incident and hot spot
              ...
              if(miles != -1.0) { // miles == -1.0 would mean hot spot and incident were not on the same road
                ...
                if(miles <= mileage) { // incident falls within radius of hot spot
                  ...
                  if(closestHotspot.isEmpty) { // not yet found hot spot close enough
                    closestHotspot = Some((currentHotspotsArr(i)._1,(miles, currentHotspotsArr(i)._2._2)))
        ...
        val updatedIncident = if (closestHotspot.isEmpty) incident else (closestHotspot.get._1, (1, closestHotspot.get._2._2, false))
        allocatedIncidents += updatedIncident
      }

      val resultVal: (Int, Array[((Double, Double), (Int, Array[String], Boolean))]) = (thisPartitionIndex, allocatedIncidents.toArray)
      val resultIter: Iterator[(Int, Array[((Double, Double), (Int, Array[String], Boolean))])] = Iterator(resultVal)

      resultIter
    } else {

      val resultVal = data
      val resultIter = Iterator(resultVal)

      resultIter
    }
  }

  /** Removes redundancies within a set of incidents within a single partition
    * I.e. Stage 2.A of fully parallel version
    *
    * @param iter iterator of incident information ((lat,lng), (n, arr, b))
    *                  where  n represents number of incidents represented by this location
    *                         arr is the array of information about this incident
    *                         b is the boolean tag -- true if this is an isolated incident i.e. not yet suppressed by a hotspot
    * @param index index of the partition
    * @param mileage radius of hotspots
    */
  def removeRedundanciesInPartition(iter: Iterator[((Double, Double), (Int, Array[String], Boolean))],
                                    index: Int,
                                    mileage: Double): Iterator[(Int, Array[((Double, Double), (Int, Array[String], Boolean))])] = {

    println("REMOVE REDUNDANCIES IN PARTITION ------------" + index)

    val centres = mutable.HashMap.empty[(Double,Double),(Int, Array[String], Boolean)] // stores resulting hotspot centres and their fitnesses / boolean flag
    val incidentsArr: Array[((Double, Double), (Int, Array[String], Boolean))] = iter.toArray
    ...
    for(i <- 0 until incidentsArr.size) { // i = incident to allocate
      ...
      if(!centres.contains(incidentsArr(i)._1)) { // if incident is already a centre, don't bother comparing it to anything => once it is a centre, cannot be allocated to another hot spot
        ...
        if(closestIncident.isEmpty) {
          ...
          val replaceCentre = (centreLocation,(newFitness, centres(centreLocation)._2, false))
          ...
          val newCentre = (centreLocation,(newFitness, incidentsArr(closestIndex)._2._2, false))
    ...
    val centresArr: Array[((Double, Double), (Int, Array[String], Boolean))] = centres.toArray
    val returnVal: (Int, Array[((Double, Double), (Int, Array[String], Boolean))]) = (index, centresArr)
    val returnIter: Iterator[(Int, Array[((Double, Double), (Int, Array[String], Boolean))])] = Iterator(returnVal)
  ...
  /** Removes redundancies within a set of incidents - used in prestage1
    * Collects RDD back to driver so must be small set of incidents
    *
    * @param incidents RDD of incident information ((lat,lng), n)
    *                  where n represents number of incidents represented by this location
    */
